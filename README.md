...
## 使用 Docker 启动（容器内自动反代 HTTPS 后端）

Docker 镜像内会启动三个进程：
- `serve`：仅在容器内部提供前端静态站点（HTTP）
- `auth-server`：校验账号密码并签发登录 Cookie
- `haproxy`：对外暴露 HTTPS 端口，并将 `/1.0`、`/oidc`、`/documentation`、`/os` 反向代理到 HTTPS 后端（携带固定客户端证书）

默认后端地址：`host.docker.internal:8443`（访问宿主机上的 Incus HTTPS API）。
默认对外端口：容器内 `443`（示例映射到主机 `8444`）。

### 1. 构建镜像

```bash
docker build -t incus-ui:local .
```

### 2. 为 8443 后端生成并信任客户端证书

```bash
# 生成给 UI 网关使用的客户端证书
openssl req -x509 -newkey rsa:2048 \
  -keyout /tmp/incus-ui-client.key \
  -out /tmp/incus-ui-client.crt \
  -days 3650 -nodes -subj "/CN=incus-ui-gateway"

# 将客户端证书加入 Incus 信任列表（后端 8443）
incus config trust add /tmp/incus-ui-client.crt

# 导出 Incus 服务端证书（用于网关校验后端证书）
sudo cat /var/lib/incus/server.crt > /tmp/incus-server-ca.crt
```

### 3. 直接运行（docker run）

```bash
docker run -d --name incus-ui \
  -p 8444:443 \
  ...
  -e BACKEND_PORT=8443 \
  -e BACKEND_SSL_VERIFY=required \
  -e BACKEND_CA_CERT_PEM="$(cat /tmp/incus-server-ca.crt)" \
  -e BACKEND_CLIENT_CERT_PEM="$(cat /tmp/incus-ui-client.crt)" \
  -e BACKEND_CLIENT_KEY_PEM="$(cat /tmp/incus-ui-client.key)" \
  incus-ui:local
```

访问：`https://<你的主机IP>:8444/ui/login`  
登录页使用你设置的 `BASIC_AUTH_USERNAME/BASIC_AUTH_PASSWORD`。

### 4. 使用 docker compose

项目已提供 `docker-compose.yml`。将证书内容填入对应环境变量后启动：

```bash
docker compose up -d --build
```

也可以直接使用一键脚本（默认执行构建+启动）：

```bash
# 全自动：初始化证书 + trust add + 构建启动
./docker/local-docker.sh bootstrap

# 首次：初始化证书（生成客户端证书，并尝试导出后端 CA）
./docker/local-docker.sh init-certs

# 默认：up（build + run）
./docker/local-docker.sh

# 仅构建
./docker/local-docker.sh build

# 停止
./docker/local-docker.sh down
```

脚本默认从以下路径读取证书（可用环境变量覆盖）：

- `docker/certs/incus-server-ca.crt`
- `docker/certs/incus-ui-client.crt`
- `docker/certs/incus-ui-client.key`

首次生成客户端证书后，请确保执行过：

```bash
incus config trust add docker/certs/incus-ui-client.crt
```

### 5. 前端 HTTPS 证书（可选）

默认会自动生成前端自签名证书。你也可以自定义：
...
docker run -d --name incus-ui \
  ...
  -e BACKEND_CLIENT_CERT_PEM="$(cat /tmp/incus-ui-client.crt)" \
  -e BACKEND_CLIENT_KEY_PEM="$(cat /tmp/incus-ui-client.key)" \
  -e PUBLIC_TLS_CERT_PEM="$(cat /path/to/fullchain.pem)" \
  -e PUBLIC_TLS_KEY_PEM="$(cat /path/to/privkey.pem)" \
  incus-ui:local
```
