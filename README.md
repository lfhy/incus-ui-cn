...
- `haproxy`：对外暴露 HTTP 端口，并将 `/1.0`、`/oidc`、`/documentation`、`/os` 反向代理到你的 HTTPS 后端

默认后端地址为：`host.docker.internal:8443`（用于访问宿主机上的 Incus HTTPS API）。
...
项目已提供 `docker-compose.yml`，按需修改 `BACKEND_HOST` 后启动：
...
- `BACKEND_SSL_VERIFY`：`none`（默认）或 `required`
- `BACKEND_HOST`：默认 `host.docker.internal`；如果后端不在宿主机，请改成实际地址
- `BACKEND_CA_CERT_PEM`：当 `BACKEND_SSL_VERIFY=required` 时，可传入后端 CA 证书内容（PEM）
- `BACKEND_CLIENT_CERT_PEM` + `BACKEND_CLIENT_KEY_PEM`：如后端要求 mTLS，可传入客户端证书与私钥（PEM）
...
docker run -d --name incus-ui \
  ...
  -e BACKEND_CA_CERT_PEM="$(cat ca.crt)" \
  -e BACKEND_CLIENT_CERT_PEM="$(cat client.crt)" \
  -e BACKEND_CLIENT_KEY_PEM="$(cat client.key)" \
  incus-ui:local
```
